...
class IQADataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print(self.data_dict['r_img_list'][idx])
        r_img_name = self.data_dict['r_img_list'][idx]
        # print(r_img_name)
        r_img = cv2.imread(os.path.join(self.db_path+"reference_images", r_img_name), cv2.IMREAD_COLOR)
        r_img = self.Random_crop(r_img)
        ...
        try:
            ...
            d_img = self.Random_crop(d_img)
        except:
            print(os.path.join(self.db_path, d_img_name))
            import pdb
            pdb.set_trace()
    ...
    def load_data_dict(self):
        ...
        with open(self.txt_file_name, 'r') as listFile:
            for line in listFile:
                score, dis, ref, scn_idx = line.split(',')
                scn_idx = int(scn_idx)
                score = float(score)
