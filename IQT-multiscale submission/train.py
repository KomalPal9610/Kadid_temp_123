...
config = Config({
    ...
    "GPU_ID": "0",
    "num_workers": 0,
    "scale":3, #could be changed as per requirement available options 0.5,1,2,3 where 0.5 is upscale by 2 and 2 and 3 are downscale by 2 and 3 factor
    ...
    # data
    "db_name": "Kadid",                                                     # database name [ PIPAL | LIVE | CSIQ | TID2013 ]
    "db_path": "/home/vj/Documents/kadid10k/images",         # root of dataset
    "snap_path": "./weights/Kadid_scale3_8020",                                         # path for saving weights
    "txt_file_name": "/home/vj/PycharmProjects/IQA_final/IQA-multiscaling-main/IQT-multiscale submission/IQA_list/Kadid.txt",                    # image list file (.txt)
    "train_size": 0.8,
    "scenes": "all",
...
elif config.db_name == 'TID':
    from data.data_TID import IQADataset
elif config.db_name == 'Kadid':
    from data.data_Kadid import IQADataset
...
if config.scale != 1:
    ...
    train_dataset = IQADataset(
        ...
        transform=transforms.Compose(
            [Normalize(0.5, 0.5), RandHorizontalFlip(), RandRotation(), ToTensor()]),
        train_mode=True,
        scene_list=train_scene_list,
        train_size=config.train_size
    )
